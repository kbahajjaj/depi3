...
def delete_employee(id):
    ...
    if not found:
        ...
        print(f"Employee {name} - ID: [{id}] was successfuly deleted from database")
    ...
    with open('employees.csv', 'r', newline='') as f:
        ...
        for row in reader:
            if id == (row[0]):
                found = True
                if "" in updates:
                    updates = [updates[i] if updates[i] != "" else row[i] for i in range(len(updates))]
                rows.append(updates)
            else:
                rows.append(row)
